...
import { FBFrom, Page, Template } from "@/interfaces/fb";
import { Offer } from "@/interfaces/kt";
...
const ConnectForm = () => {
  ...
  useEffect(() => {
    const fetchPages = async () => {
      ...
      const data = await getPages(fb_access_token as string);
      setDto((prev) => ({ ...prev, pages: data }));
    };

    const fetchGetTemplates = async () => {
      const userId: string | null = Cookies.get("userId");
      const temp = await getTemplates(userId as string);
      if (temp.data) {
  ...
  useEffect(() => {
    const fetchForms = async () => {
      const selectedPage = dto.pages.find((p: any) => p.name === dto.page);
      ...
      setDto({ ...dto, pageForms: forms, pageId: String(id) });
    };
  ...
  const subscribePage = async () => {
    ...
    const { access_token, id }: any = dto.pages.find(
      (p) => p.name === dto.page
    );
      ...
      <form className="w-full md:w-1/2 bg-white rounded-2xl p-6 shadow-xl flex flex-col gap-6 border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 justify-items-center">
          <Dropdown
            ...
            value={dto.page}
            onValueChange={(val) => setDto({ ...dto, page: String(val) })}
          ...
          {dto.pageForms.length > 0 && (
            <Dropdown
              ...
              value={dto.form}
              onValueChange={(val) => setDto({ ...dto, form: val })}
          ...
          <Dropdown
            ...
            value={dto.type}
            onValueChange={(val) => setDto({ ...dto, type: val })}
          ...
          <Dropdown
            options={[]}
            value={dto.ad}
            onValueChange={(val) => setDto({ ...dto, ad: val })}
            placeholder="Adset"
          />
        </div>
        ...
        {dto.type === "GOOGLESheets" && (
          <div className="grid grid-cols-2 gap-4">
            <Dropdown
              options={[]}
              value={dto.tableId}
              onValueChange={(val) => setDto({ ...dto, tableId: val })}
              placeholder="Table ID"
            />
            <Dropdown
              options={[]}
              value={dto.sheet}
              onValueChange={(val) => setDto({ ...dto, sheet: val })}
              placeholder="Sheet"
            />
          </div>
        )}
        {dto.type === "CRM" && (
          <div className="grid grid-cols-2 gap-4">
            <Dropdown
              ...
              value={dto.source}
              onValueChange={(val) => setDto({ ...dto, source: val })}
              placeholder="source"
            />
            <Dropdown
              ...
              value={String(dto.user_id)}
              onValueChange={(val) => setDto({ ...dto, user_id: Number(val) })}
              placeholder="Aff ID"
            />
            <Dropdown
              options={dto.offers.map((o) => o.name)}
              value={dto.offer}
              onValueChange={(val) => setDto({ ...dto, offer: val })}

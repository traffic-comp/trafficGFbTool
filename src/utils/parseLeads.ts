...
export function extractAnswers(lead: any) {
  const allKeywords = [
  ...
  return lead.field_data
    ...
    .map((val: any) => val.replace(/\s+/g, " ").trim())
    .join("; ");
}

export function getFieldValueByKeywords(lead: any, keywords: any) {
  const normalizedKeywords = keywords.map(normalize);

  const match = lead.field_data.find((f: any) => {
    const nameNormalized = normalize(f.name);
